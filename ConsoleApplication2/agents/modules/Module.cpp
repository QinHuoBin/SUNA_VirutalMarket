...
#define pdt2() print_delta_time2(__LINE__);
void print_delta_time2(int line_num)
{
	return;
...
void Module::clone(Module *brother)
{
	...
	//printf("brother->allocated_space=%d\n",brother->allocated_space);
	if (brother->allocated_space > 100000)
	{
		printf("brother->allocated_space异常\n");
	}
...
void Module::reallocEverything(int given_allocated_space)
{
// 理解错误！这个函数不能用，否则会造成数组越界！
// 原因：realloc后，原来的数据没有被清除！
exit(-1);
	//double the previous allocated space
	int previous_space = allocated_space;
	allocated_space = given_allocated_space;
	...
	//printf("given_allocated_space=%d\n",given_allocated_space);
	if(given_allocated_space>1000000)
	{
		printf("异常given_allocated_space=%d\n",given_allocated_space);
	}

	// 需要申请多大的内存
	int bytes_to_malloc =
		allocated_space * sizeof(neuron) + allocated_space * sizeof(connection) + allocated_space * sizeof(double) + allocated_space * sizeof(double) + allocated_space * sizeof(double) + allocated_space * sizeof(double) + allocated_space * sizeof(bool) + allocated_space * sizeof(int) + allocated_space * sizeof(int) + allocated_space * sizeof(int);

	// 如果还未把申请的内存区域整合成一块，先free，再申请一大块内存
	if (!have_improved)
	{
		free(n);
		free(c);
		...
		malloc_area = calloc(bytes_to_malloc,1);
		have_improved = true;
	}
	else
	{
		// 否则free掉原来的区域，重新申请
		free(malloc_area);
		calloc(bytes_to_malloc,1);
	}

	int point_now = 0;

	// n = (neuron *)realloc(n, allocated_space * sizeof(neuron));
	n = (neuron *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(neuron);

	// c = (connection *)realloc(c, allocated_space * sizeof(connection));
	c = (connection *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(connection);

	// neuron_state = (double *)realloc(neuron_state, allocated_space * sizeof(double));
	neuron_state = (double *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(double);

	// previous_neuron_state = (double *)realloc(previous_neuron_state, allocated_space * sizeof(double));
	previous_neuron_state = (double *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(double);

	// internal_neuron_state = (double *)realloc(internal_neuron_state, allocated_space * sizeof(double));
	internal_neuron_state = (double *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(double);

	// neuron_excitation = (double *)realloc(neuron_excitation, allocated_space * sizeof(double));
	neuron_excitation = (double *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(double);

	// is_fired = (bool *)realloc(is_fired, allocated_space * sizeof(bool));
	is_fired = (bool *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(bool);

	// primer_list = (int *)realloc(primer_list, sizeof(int) * (allocated_space));
	primer_list = (int *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(int);

	// connection_state = (int *)realloc(connection_state, allocated_space * sizeof(int));
	connection_state = (int *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(int);

	// previous_connection_state = (int *)realloc(previous_connection_state, allocated_space * sizeof(int));
	previous_connection_state = (int *)(malloc_area + point_now);
	point_now += allocated_space * sizeof(int);

	if(point_now!=bytes_to_malloc)
	{
		printf("分配的空间不准确！%d %d",point_now,bytes_to_malloc);
		exit(-1);
	}
