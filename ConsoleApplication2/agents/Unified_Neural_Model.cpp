...
void save_individual(Module *individual,double this_tmp_fitness,int order, string base_path)
{
	...
	string name = to_string(this_tmp_fitness + 10000) + '_' + to_string(order);

	string dna_path = base_path + dna + name + ".dna";
	individual->saveDNA(dna_path.c_str());

	string dot_path = base_path + dot + name + ".dot";
	individual->printGraph(dot_path.c_str());
...
void Unified_Neural_Model::save_all_agents()
{
	...
	subpopulation[MAIN_SUBPOP][best_index]->saveDNA(best_individual_dna_path.c_str());
	subpopulation[MAIN_SUBPOP][best_index]->printGraph(best_individual_dot_path.c_str());
	system(("dot " + best_individual_dot_path + " -Tpng -o " + best_individual_png_path).c_str());
...
void Unified_Neural_Model::step_my(double *observation, double reward, int the_individual)
{
	//execute individual
	subpopulation[testing_subpop][the_individual]->process(observation, action);
...
void Unified_Neural_Model::endEpisode_my(double reward, int the_individual)
{
	//update reward
	tmp_fitness[testing_subpop][the_individual] += reward;
...
#define pdt() print_delta_time(__LINE__);
void print_delta_time(int line_num)
{
	return;
